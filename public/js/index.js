(function(exports) {
    ...
    function permalinks_func(ref_link) {
        if (ref_link.indexOf("http") != -1 || ref_link.indexOf("https") != -1) {
            return ref_link;
        }
        return "http://" + ref_link;
    };
    ...
    var BLOCKS = {
        ...
        "today": renderBlock()
	         ...
	         .render(function(svg, data) {
	            ...
	            var series = timeSeries()	            
	                         .series([data.data])
	                         .y(y)
	                         .label(function(d) { return formatHour(d.hour); })
	                         .title(function(d) { return formatCommas(d.visits) + " visits during the hour of " + formatHour(d.hour) + "m";})
	                         .xScale().domain(d3.range(0, days.length + 1))
	                         .yScale().domain([0, d3.max(days, y)])
	                         .yAxis().tickFormat(formatVisits);
        ...
        */
        ...
        .on("render", function(selection, data) {
            ...
            d3.select("#total_visitors-7")
                .text(formatBigNumber(total));
        }),
        "devices_30": renderBlock()
        .transform(function(d) {
            var devices = listify(d.totals.devices);
            return addShares(devices);
        })
        ...
        .transform(function(d) {
            var values = listify(d.totals.browser),
                total = d3.sum(values.map(function(d) {
                    return d.value;
                }));
        ...
        "ie": renderBlock()
        .transform(function(d) {
            ie_list_filtered = addShares(listify(d.totals.ie_version));
            return ie_list_filtered.slice(0, 4);
        })
        .render(
            barChart()
            .value(function(d) {
        ...
        .transform(function(d) {
            ...
            city_list_filtered = addShares(city_list_filtered, function(d) {
        ...
        .transform(function(d) {
            // remove "(not set) from the data"
            var countries_list = d.data;
            var countries_list_filtered = countries_list.filter(function(c) {
                return (c.country != "(not set)") && (c.country != "zz");
            });
            countries_list_filtered = addShares(countries_list_filtered, function(d) {
                return d.active_visitors;
            });
            return countries_list_filtered.slice(0, 10);
        })
        .render(
            barChart()
            .value(function(d) {
        ...
        "referrers": renderBlock()
        .transform(function(d) {
            // remove "(not set) from the data"
            var referrers_list = d.data;
            var referrers_list_filtered = referrers_list.filter(function(c) {
                return (c.referrer != "(not set)") && (c.referrer != "zz");
            });
            referrers_list_filtered = addShares(referrers_list_filtered, function(d) {
                return d.instances;
            });
            return referrers_list_filtered.slice(0, 10);
        })
        .render(
            barChart()
            .value(function(d) {
        ...
        .render(
            ...
            .label(function(d) {
                ...
                */ 
                return [
                    '<span class="filename"><a class="top-download-file" target="_blank" href=http://', d.event_label, '>', d.event_title, '</a></span>',
                    '<span class="divider"> | </span> ',
        ...
        .on("render", function(selection, data) {
            ...
            selection.selectAll(".label")
                ...
                .attr("title", function(d) {
                    return exceptions[d.page_title] || d.page;
                })
                .attr("href", function(d) {
                    return exceptions[d.page] || permalinks_func(d.page);
                })
                .text(function(d) {
        ...
        .on("render", function(selection, data) {
            ...
            selection.selectAll(".label")
                ...
                .attr("target", "_blank")
                .attr("title", function(d) {
                    return exceptions[d.page_title] || d.page_title;
                })
                .attr("href", function(d) {
                    return exceptions[d.page] || permalinks_func(d.page);
                })
    ...
    // nest the IE chart inside the browsers chart once they're both rendered
    whenRendered(["browsers-7", "ie"], function() {
        d3.select("#chart_browsers-7")
            .call(nestCharts, function(d) {
                return d.key === "Internet Explorer";
            }, d3.select("#chart_ie-7"));
    });
    // nest the IE chart inside the browsers chart once they're both rendered
    whenRendered(["browsers-30", "ie"], function() {
        d3.select("#chart_browsers-30")
            .call(nestCharts, function(d) {
                return d.key === "Internet Explorer";
            }, d3.select("#chart_ie-30"));
    });
    ...
    */
    ...
    d3.selectAll("*[role='tablist']")
        .each(function() {
            ...
            var tabs = d3.select(this).selectAll("*[role='tab'][href]")
                .datum(function() {
                    var href = this.href, target = document.getElementById(href.split("#").pop());
    ...
    /*function timeSeries() {
        ...
        var margin = { top: 10, right: padding, bottom: 25, left: padding };
            
        var x = function(d, i) { return i; };
        
        var y = function(d, i) { return d; };
        
        var label = function(d, i) { return i; };
        
        var title = function(d) { return d; };
        ...
        var timeSeries = function(svg) {
        	...
        	var left = margin.left, right = width - margin.right, top = margin.top, bottom = height - margin.bottom;
    ...
    // Set the dropdown
    var dropDowns = document.getElementsByClassName('agency-selector-cls');
